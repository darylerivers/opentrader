...
def generate_simulated() -> dict:
    ...
    unemployment = {
        "low": 3.5, "mid": 4.5, "high": 6.0,
    }[regime["rates"]] + rng.uniform(-0.3, 0.3)

    cpi_yoy = {
        "low": 2.0, "mid": 3.0, "high": 5.5,
    }[regime["rates"]] + rng.uniform(-0.5, 0.5)

    gdp_growth = {
        "low": 1.5, "mid": 2.5, "high": 1.0,
    }[regime["rates"]] + rng.uniform(-0.3, 0.3)

    indicators = [
        {"name": "Fed Funds Rate", "value": round(rate, 2), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
        {"name": "10Y Treasury", "value": round(ten_yr, 2), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
        {"name": "2Y Treasury", "value": round(two_yr, 2), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
        {"name": "10Y-2Y Spread", "value": round(sp, 2), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
        {"name": "Unemployment", "value": round(unemployment, 1), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
        {"name": "CPI (YoY)", "value": round(cpi_yoy, 1), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
        {"name": "GDP Growth", "value": round(gdp_growth, 1), "unit": "%", "date": datetime.now().strftime("%Y-%m-%d")},
    ]
