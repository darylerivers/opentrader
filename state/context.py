...
class AccountContext:
    ...
    def fee_impact(self, symbol: str, notional: float) -> tuple:
        ...
        rt = buy + sell
        return (round(buy, 2), round(sell, 2), round(rt, 2), round(rt / notional * 100, 1) if notional > 0 else 0.0)
    ...
    def summary_json(self) -> dict:
        ...
        for route_name in sorted(set(self.route_map.values()) | {self.exchange}):
            ...
            if fs.taker_pct > 0:
                lines.append(f"  {route_name}: {fs.taker_pct:.2%} taker / {fs.maker_pct:.2%} maker")
            elif fs.fixed_per_trade > 0:
                lines.append(f"  {route_name}: ${fs.fixed_per_trade:.2f} per trade + ${fs.min_fee:.2f} min")

        # Minimum position advice
        lines.append(f"Min position: ${self.min_position_notional('default'):.2f} (keep fees under 20%)")
        lines.append(f"Commission impact: Aim for positions >5x the round-trip commission cost.")
