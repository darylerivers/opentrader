...
def _build_pva(num_points: int = 500) -> dict:
    ...
    import time as _t
    now = _t.time()
    if (_PVA_CACHE["data"] is not None and _PVA_CACHE["n"] == num_points
    ...
    files = files[-num_points:]  # take most recent N
    ...
    for fpath in files[::sample_n]:
        ...
        if base_pv is None:
            base_pv = pv
        pt = {"ts": str(ts)[:19],
              "portfolio_pct": round((pv / max(base_pv, 0.01) - 1) * 100, 2),
        ...
        for sym, px in prices.items():
            ...
            if sym_short in base_prices and base_prices[sym_short] > 0:
                pt[sym_short] = round(
                    (px / base_prices[sym_short] - 1) * 100, 2)
        points.append(pt)
...
async def history_summary():
    ...
    for fpath in files:
        ...
        rows.append({
            ...
            "pnl_pct": round(
                (d.get("portfolio_value", 0) / max(d.get("initial_cash", 1), 1) - 1) * 100, 2
            ),
        })
...
async def api_trades():
    s = _read_state()
    trades = s.get("trades", [])
    return {"trades": trades[-30:], "count": len(trades),
    ...
    async def gen():
        while True:
            ...
            summary = {
                ...
                "pv": s.get("portfolio_value", 0),
                "cash": s.get("cash", 0),
                "pnl_pct": round((s.get("portfolio_value", 0) / max(
                    s.get("initial_cash", 100_000), 1) - 1) * 100, 2),
                "positions": len(s.get("positions", [])),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=8097, help="Port to listen on (default: 8097)")
