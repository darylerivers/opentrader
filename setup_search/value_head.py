...
def collect(closes, highs, lows, vols, cfg):
    ...
    for sym in sorted(closes.keys()):
        ...
        for t in range(len(c)):
            ...
            if s != s or s < cfg["buy_thresh"]:  # NaN != NaN excludes warmup bars
                continue
            regime_ok = True
...
def main():
    ...
    best_val, best_state, patience, it = -1e9, None, 0, 0
    for epoch in range(120):
        ...
        opt.step()
        model.eval()
        d, nkeep, _, _ = discrim(val, model, 0.0, (mean, std))
        if d > best_val:
            best_val, best_state, patience = d, {k: v.clone() for k, v in model.state_dict().items()}, 0
        else:
            patience += 1
            if patience >= 10:
                break
    ...
    model.eval()
    print(f"[v] trained {it+1} epochs; val discrimination {best_val:+.2%}")

    results = []
    for lo, hi in TESTS:
        win = [r for r in all_rows if lo <= r["bar"] < hi]
        d, nkeep, kept_m, all_m = discrim(win, model, 0.0, (mean, std))
        results.append({"window": f"{lo}-{hi}", "n": len(win), "kept": nkeep,
                        "kept_mean": kept_m, "all_mean": all_m, "margin": d})
        print(f"[v] window {lo}-{hi}: n={len(win)} kept={nkeep} "
    ...
    (OUT / "value_head_report.json").write_text(json.dumps({
        ...
        "n_train": len(trn), "n_val": len(val), "val_discrimination": best_val,
        "results": results, "pass": pass_gate}, indent=1, default=str))
